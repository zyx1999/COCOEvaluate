...
def soft_nms(np.ndarray[float, ndim=2] boxes, float sigma=0.5, float Nt=0.3, float threshold=0.001, unsigned int method=0):
    # shape of "boxes" : (2000, 7)
    # boxes[i][0] = bboxes[i][0]   tl_x <=> tx1
    # boxes[i][1] = bboxes[i][1]   tl_y <=> ty1
    # boxes[i][2] = bboxes[i][2]   br_x <=> tx2
    # boxes[i][3] = bboxes[i][3]   br_y <=> ty2
    # boxes[i][4] = scores[i][0] score <=> ts
    # boxes[i][5] = tl_scores[i][0] 在soft_nms中没有用到
    # boxes[i][6] = br_scores[i][0] 在soft_nms中没有用到
